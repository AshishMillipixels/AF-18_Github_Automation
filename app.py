...
class CloneRequest(BaseModel):
    repo_url: str = Field(..., example="https://github.com/username/repo.git")
    local_path: str = Field(..., example="./cloned-repo")
...
async def create_github_pr(request: PRRequest):
    try:
        ...
        # Handle specific ValueError with a 400 status code
        logger.error(f"Invalid request: {ve}")
        raise HTTPException(status_code=400, detail=str(ve))
    except Exception as e:
        # Generic exception handler
        logger.error(f"Unexpected error creating pull request: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error creating pull request")
...
async def clone_repo(request: CloneRequest):
    try:
        if os.path.exists(request.local_path):
            raise HTTPException(400, "Target path already exists")
            
        repo = git.Repo.clone_from(
            url=request.repo_url,
            to_path=request.local_path
        )
        return {
            "success": True,
            "message": f"Cloned to {request.local_path}",
